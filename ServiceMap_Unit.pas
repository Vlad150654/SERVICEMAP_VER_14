...
type
  ...
  private
    ...
    procedure Intersects(var id, row, radius: Integer; var fields: TStringList;
      var WorkSheet, DataParcels: OleVariant; tab, tname, fname: String);
...
begin
  ...
  ExcelApp.Workbooks.Open
    ('D:\Tokyo\SERVICEMAP_VER_14\Win32\Debug\TEST\Telda2НСПД\Примеры файлов из Телда\Книга1.xlsx');
...
begin
  ...
  if not FileExists(GetAppFolder +
    'TopnLab\Справочник соответствия модели и ЦОФ.xlsx') then
...
{$IFDEF DEBUG}sourcepath := 'D:\Tokyo\SERVICEMAP_VER_14\Win32\Debug\TEST\Telda2НСПД\Примеры файлов из Телда\'; {$ENDIF}
  ...
  try
    ...
    ExcelApp.DisplayAlerts := false;
    try
      SafeCloseTable(miEval('PathToTableName$("%s")',
        [GetAppFolder + 'TopnLab\Справочник соответствия модели и ЦОФ.tab']));
      tab := CreateTableFromExcel(GetAppFolder +
        'TopnLab\Справочник соответствия модели и ЦОФ.xlsx', 'Справочник', '');
      miDo('Select Код_Модели, count(*) from %s group by Код_Модели into groupTable',
        [tab]);
      ...
      begin
        ...
        cof.Add(miEval('%s.Наименование_ЦОФ_Телда', [tab]));
        // полный список полей
      ...
      begin
        ...
        Workbook.SaveAs(Filename := s, FileFormat := xlCSV,
          CreateBackup := false);
        ExcelApp.Workbooks.Close;
        fd.LoadFromFile(s);
        DeleteFile(s);
        sa := SplitStr(fd[4], ';');
        // Из данных выгрузки берем строку со списком полей

        // проверка соответстия полей справочника и выгрузки
        for j := Low(sa) to High(sa) do
        begin
          mc := reg.Matches(sa[j], '\d\d\.\d', []);
          // ищем сигнатуру кода модели
          if mc.Count > 0 then
          begin
            kod_model := Copy(mc.Item[0].Value, 1, 2);
            // выберем в справочнике все записи по коду модели
            miDo('Select * from %s Where Код_Модели = %s into tempTab',
              [tab, kod_model]);
            for n := 1 to miEvalInt('TableInfo(tempTab, %d)',
              [TAB_INFO_NROWS]) do
            begin
              ...
              for k := Low(sa) to High(sa) do
                if Pos(s, sa[k]) = 1 then
                // проверяем по вхождению (строки в MI могут быть обрезаны!)
                  Inc(m);
  ...
  try
    ...
    begin
      ...
      begin
        ...
        s := xml.Text;
        s := StringReplace(s, '</definedName>', '</definedName>' + #10,
          [rfReplaceAll]);
        ...
        begin
          ...
          xml.SaveToFile(sl[i]);
          Zip.Close;
          s := ExtractFilePath(miEval('TempFileName$("")')) +
            ExtractFileName(xlsname);
          Zip.ZipDirectoryContents(s, TempPath, zcDeflate);
          Zip.Close;
          // ExShellExecute('cmd', format('/C xCopy /b /Y "%s" "%s"',
          ExShellExecute('cmd', format('/C Copy /b /Y "%s" "%s"', [s, xlsname]),
            SW_HIDE, true, 0);
          exit;
        end;
      end;
    ...
    ExShellExecute('cmd.exe', s, SW_HIDE, true, 0);
    // удалене каталога со всем содержимым и подкаталогами
  finally
    sl.Free;
    xml.Free;
    Zip.Free;
...
{$IFDEF DEBUG} ServiceMapForm.OpenDialog1.InitialDir := 'D:\Tokyo\SERVICEMAP_VER_14\Win32\Debug\Новый источник данных'; {$ENDIF}
  ...
  try
    ...
    for j := VarArrayLowBound(DataParcels, 1) + 1 to VarArrayHighBound
      (DataParcels, 1) do
    begin // создаем запросы по таблицам
      ss := DataParcels[j, 1];
      if (ss <> '') and (ss[1] = '#') then
      // дополнительные поля берем из таблицы Mahinfo
      begin
    ...
    for i := VarArrayLowBound(DataParcels, 1) to VarArrayHighBound
      (DataParcels, 1) do
    begin // идем по строкам
      if DataParcels[i, 1] = DataParcels[i, 2] then
      begin
        k := i + 1; // подсчет ячеек для объединения
        while (k <= VarArrayHighBound(DataParcels, 1)) and
          (DataParcels[k, 1] = '') do
          Inc(k);
    ...
    mc := reg.Matches(s, '<offer internal-id="\d{7,}">.+</offer>', [roMultiLine]
      ); // <offer internal-id="102080425111">
    if mc.Count = 0 then
    ...
    begin
      ...
      for j := VarArrayLowBound(DataParcels, 1) + 1 to VarArrayHighBound
        ...
        if DataParcels[j, 1] = DataParcels[j, 2] then
          s := StringReplace(s, DataParcels[j, 1], lf + DataParcels[j, 1],
            [rfReplaceAll]);
      ...
      for j := VarArrayLowBound(DataParcels, 1) + 1 to VarArrayHighBound
        ...
        begin // создаем запросы
          ...
          begin
            if DataParcels[j, 1] = DataParcels[j, 2] then
            // выбираем вложенные узлы
            begin
              for n := j + 1 to VarArrayHighBound(DataParcels, 1) do
                if DataParcels[n, 1] = '' then
                // выбираем аттрибуты узла используя пустые значениея
                begin
                  ss := DataParcels[n, 2];
                  System.Insert('/', ss, 2);
                  k := Length(ss);
                  ...
                  if mc1.Count > 0 then
                  begin
                    Data[1, n - m] := Copy(mc1.Item[0].Value, k,
                      Length(mc1.Item[0].Value) - k - k + 1); // данные из XML

                    if (n - m = lot_area + 1) and
                      CText(Data[1, lot_area + 1], 'сотка') then
                    begin
                      Val(Data[1, cad_area], area, p);
                      // площадь взятая из объекта MI
            ...
            else // выбираем данные при узле
            begin
              Data[1, j - m] := Copy(sub.Item[0].Value, k,
                Length(sub.Item[0].Value) - k - k + 1);
              if CText(DataParcels[j, 1], '<description>') then
              begin
                ss := Data[1, j - m];
                ss := StringReplace(ss, '<br />', '', [rfReplaceAll]);
                ss := StringReplace(ss, format('<![CDATA[Арт. %s ', [Data[1, 1]]
                  ), '', [rfReplaceAll]);
                ...
                begin // в описании ищем кадастровый номер
                  mc1 := reg.Matches(ss, '\d\d:\d\d:\d{1,7}:\d{1,}',
                    [roMultiLine]);
          ...
          else if CText(DataParcels[j, 1], '<lot-area>') then
          // пллощадь по лоту не найдена
          ...
          begin
            ...
            begin
              miDo('Select * from Дежурная_карта where obj Contains ob Into TempTab NoSelect');
              // центроид объекта B лежит в границах объекта A
              if miEvalInt('TableInfo(TempTab, %d)', [TAB_INFO_NROWS]) > 0 then
              begin
                Data[1, j - m] := miEval('TempTab.КН');
                Data[1, cad_area] :=
                  format('%.f', [miEvalDbl('CartesianArea(TempTab.obj, "sq m")')
                  / 100]); // площадь в сотках
              end;
            end;
          end;
        end;
      ...
      begin // создаем запросы по таблицам MI
        ...
        begin
          ss := Copy(ss, 2, Length(ss));
          miDo('Select * from %s where obj Contains ob Into TempTab NoSelect',
            [ss]); // центроид объекта B лежит в границах объекта A
          if miEvalInt('TableInfo(TempTab, %d)', [TAB_INFO_NROWS]) > 0 then
          begin
            if DataParcels[j, 2] <> '' then // есть дополнительные поля
            begin
              Data[1, j - m] := miEval('TempTab.%s', [DataParcels[j, 2]]);
              // берем значение этого поля
              while j < VarArrayHighBound(DataParcels, 1) do
              // продолжаем брать из других полей
              begin
                if (j < VarArrayHighBound(DataParcels, 1)) and
                  (DataParcels[j, 1] = DataParcels[j + 1, 1]) then
                begin
                  Inc(j);
                  Data[1, j - m] := miEval('TempTab.%s', [DataParcels[j, 2]]);
                end
            ...
            else // для таблиц без полей (Буфер_от500, water_polygon)
              Data[1, j - m] := 1; // есть вхождение
          end
          else if DataParcels[j, 2] = '' then
          // для таблиц без полей (Буфер_от500, water_polygon)
...
begin
  ini := TMemIniFile.Create(GetAppFolder + 'ServiceMap.ini');
  try
    CatZip.Text := ini.ReadString('sevenzip', 'path',
...
var
  ...
  s, s2: String;
begin
{$IFDEF DEBUG}path := 'D:\Tokyo\SERVICEMAP_VER_14\Win32\Debug\TEST\для расчета от шмелева 2026_09_102+026_09_147\'; {$ENDIF}// для расчета от шмелева 2026_09_10
    ...
    for i := 0 to sl.Count - 1 do
      if CText(ExtractFileName(sl[i]), 'All') then
        Exc('В выбранном каталоге имеется папка "All"' + #10 +
          'в которую заносится результаты объединения.' + #10 +
          'Удалите эту папку.');
  finally
    sl.Free;
  end;
...
begin
  ...
  try
    Info(' Выбор объектов..');
    miDo('select * from %s Order By КН into work NoSelect', [tab]);
    // выбираем объекты
    ...
    ExCreateProcess
      (format('"%s" a -tzip -mx5 "%sДежурная_карта_%s.zip" "%sДежурная_карта.*"',
      [CatZip.Text, GetAppFolder + 'Base\', CurrentData, GetAppFolder + 'Base\']
      ), '', SW_HIDE, true);
    ...
    begin
      ...
      miDo('Fetch Rec %d From work', [row]);
      // выборка из таблицы AllTables или из выбранной

      cn := miEval('work.КН');
      miDo('select * from work where КН="%s" into selwork NoSelect', [cn]);
      miDo('select * from Дежурная_карта where КН="%s" into selbase NoSelect',
        [cn]);

      kz := miEvalInt('TableInfo(selwork, %d)', [TAB_INFO_NROWS]);
      // число выборок в выгрузке
      kb := miEvalInt('TableInfo(selbase, %d)', [TAB_INFO_NROWS]);
      // число выборок в базе
      arz := 0;
      arb := 0;
      try
      ...
      begin // замена
        ...
        if CText(tz, 'Region') and CText(tb, 'Point') then
        // сравниваем типы объетов
        begin // точечный объект в дежурке заменяем площадным без проверки обновления
          ...
          log.Add(format('замена%s%s%s%s%s%s%s%d%s%d%s%.2f%s%.2f%s%.2f',
            [#9, cn, #9, gmd(s_b), #9, gmd(s_z), #9, kb, #9, kz, #9, arb, #9,
            arz, #9, arb - arz]));
          miDo('Delete from selbase'); // объект в базе удаляем всегда
        end
        else if s_z > s_b then
        // сравниваем по дате, более новую выгрузку заносим в базу
        begin
          miDo('Insert Into дежурная_карта Select * From selwork');
          // пакет-загрузка
    ...
    if log.Count > 1 then
      ExShellExecute('Notepad.exe', GetAppFolder + 'Base\logUpdate.log',
        SW_NORMAL, false, 0)
    else
    ...
    begin
      miDo('Select * from Дежурная_карта Where rowid>%d into temp NoSelect',
        [MaxRow]);
...
begin
  ...
  path := 'D:\Tokyo\SERVICEMAP_VER_14\Win32\Debug\TEST\тест_правка_FD\Сервер\';
  // tr!!!
  ...
  try
    ...
    begin
      ...
      if FileExists(s) then
        if not DeleteFile(s) then
          Exc(format
            ('Файл "%s" открыт в Excel.%s Закройте файл и повторите попытку.',
            [s, lf]));
    end;
    ...
    begin
      ...
      begin
        ...
        s := '15:' + IntToStr(15 + row - 4);
        // вставляем пустые строки перед 15 строкой
        WorkSheet.Rows[s].Insert(Shift := xlShiftDown);

        // data := WorkSheet_.range[format('A3:A%d', [row])].value;
        WorkSheet.range[format('A14:A%d', [row - 3 + 14])].Value :=
          WorkSheet_.range[format('A3:A%d', [row])].Value;
        WorkSheet.range[format('A14:B%d', [14 + row - 3])].Merge(true);

        // data := WorkSheet_.range[format('B3:B%d', [row])].value;
        WorkSheet.range[format('C14:C%d', [row - 3 + 14])].Value :=
          WorkSheet_.range[format('B3:B%d', [row])].Value;

        // data := WorkSheet_.range[format('C3:C%d', [row])].value;
        WorkSheet.range[format('D14:D%d', [row - 3 + 14])].Value :=
          WorkSheet_.range[format('C3:C%d', [row])].Value;
        WorkSheet.range[format('D14:E%d', [14 + row - 3])].Merge(true);

        // data := WorkSheet_.range[format('D3:D%d', [row])].value;
        WorkSheet.range[format('F14:F%d', [row - 3 + 14])].Value :=
          WorkSheet_.range[format('D3:D%d', [row])].Value;
      ...
      begin
        ...
        s := '12:' + IntToStr(12 + row - 4);
        // вставляем пустые строки перед 12 строкой
        WorkSheet.Rows[s].Insert(Shift := xlShiftDown);

        // data := WorkSheet_.range[format('A3:A%d', [row])].value;
        WorkSheet.range[format('A11:A%d', [row - 3 + 11])].Value :=
          WorkSheet_.range[format('A3:A%d', [row])].Value;
        ...
        // data := WorkSheet_.range[format('B3:B%d', [row])].value;
        WorkSheet.range[format('C11:C%d', [row - 3 + 11])].Value :=
          WorkSheet_.range[format('B3:B%d', [row])].Value;

        // data := WorkSheet_.range[format('C3:C%d', [row])].value;
        WorkSheet.range[format('D11:D%d', [row - 3 + 11])].Value :=
          WorkSheet_.range[format('C3:C%d', [row])].Value;

        // data := WorkSheet_.range[format('D3:D%d', [row])].value;
        WorkSheet.range[format('E11:E%d', [row - 3 + 11])].Value :=
          WorkSheet_.range[format('D3:D%d', [row])].Value;
        WorkSheet.range[format('E11:F%d', [11 + row - 3])].Merge(true);

        // data := WorkSheet_.range[format('E3:E%d', [row])].value;
        WorkSheet.range[format('G11:G%d', [row - 3 + 11])].Value :=
          WorkSheet_.range[format('E3:E%d', [row])].Value;
    ...
    begin
      ...
      ExShellExecute('cmd', PChar(format('/c MOVE /Y "%s" "%s"',
        [s, xls + ExtractFileName(sl[i])])), SW_HIDE, true, 0);
      // перенести файл с переименованием
    end;
...
{$IFDEF DEBUG}path := 'D:\Tokyo\SERVICEMAP_VER_12\Win32\Debug\Письма'; {$ENDIF}    // 2024_04_27
  ...
  work := AddSlesh(AddSlesh(GetCurrentDir) + 'Графика\' +
    ExtractFileName(DellSlesh(path)));
  ...
  try
    ...
    begin
      ...
      begin
        ...
        s := StringReplace(Copy(s, 1, Length(s) - 4), '_', ':',
          [rfReplaceAll]) + ';';
        k := 0;
        for n := 0 to csv.Count - 1 do // ищем строку с КН в списке CSV
          ...
          begin
            ...
            mif.dat := StrToInt(sa[2]) * 10000 + StrToInt(sa[1]) * 100 +
              StrToInt(sa[0]);
            SetLength(sa, 0);
            break;
          end;
        if k = 0 then
          log.Add('Кадастровый номер ' + Copy(s, 1, Length(s) - 1) + ' файле ' +
            sl[j] + ' не найден в файлах CSV');
      end;
    end;
    pg.Max := 0;
    ...
    mifmid.Sort(@SortMif);
    // сортировка на увеличение даты пакета для перезаписи
    ...
    for i := 0 to sl.Count - 1 do
      try
        ...
        if i = 0 then
        begin
          miDo('select NOTE,Дата_пакета,Дата_проверки from %s into AllTables',
            [tab]);
          miDo('Commit Table AllTables As "%sAllTables.TAB" TYPE NATIVE Charset "WindowsCyrillic"',
            [work]);
          miDo('Close All');
        ...
        begin
          miDo('Insert into AllTables Select NOTE,Дата_пакета,Дата_проверки from %s',
            [tab]);
  ...
  if TableExists('AllTables') and cbMapBasic.Checked and
    FileExists(GetAppFolder + cbProgramMB.Text) then
  begin
    miDo('Open table "%s"', [GetAppFolder + 'Base\Дежурная_карта.tab']);
    ...
    begin
      ...
      // переоткрываем AllTables с другими полями из брака
      ...
      if not FileExists(GetAppFolder + 'Base\Дежурная_карта_брак.tab') then
        miDo('Commit table AllTables As "%sДежурная_карта_брак.tab"',
          [GetAppFolder + 'Base\']);
      miDo('Open table "%s"', [GetAppFolder + 'Base\Дежурная_карта_брак.tab']);

      Info(' Обновление брака...');
      miDo('select * from AllTables Order By КН into brak NoSelect');
      ...
      begin
        ...
        miDo('select * from Дежурная_карта_брак where КН="%s" into selbase NoSelect',
          [cn]);
        kz := miEvalInt('TableInfo(selbrak, %d)', [TAB_INFO_NROWS]);
        // число выборок в выгрузке
        kb := miEvalInt('TableInfo(selbase, %d)', [TAB_INFO_NROWS]);
        // число выборок в базе
        ...
        else // этого номера в базе нет, добавляем все выбранные части КН
          miDo('Insert Into дежурная_карта_брак Select * From selbrak');
        Inc(i, kz);
        pg.Position := i;
      end;
      ...
      ExShellExecute('cmd', PChar(format('/C rd /S /Q "%s"', [dir])), SW_HIDE,
        true, 0); // удаление каталога со всем содержимым и подкаталогами
      ForceDirectories(dir);
      ...
      if FileExists(format('%sГрафика_вся\брак\Самопересечение.TAB', [work]))
      then
        ExShellExecute('cmd',
          PChar(format
          ('/C Copy /b /Y "%sГрафика_вся\брак\Самопересечение.*" "%s"',
...
{$IFDEF DEBUG} path := 'D:\temp\'; {$ENDIF}
  ...
  ExCreateProcess(format('"%sNormalization.exe" "%s" "%s"', [GetAppFolder, path,
    CatZip.Text]), '', SW_NORMAL, true);

  sl := TStringList.Create;
  try
    path := Copy(path, 1, Length(path) - 1) + '_' + CurrentData + '\';
    // отсекаем слеш
    FindFolders(path, sl, false);
    for i := 0 to sl.Count - 1 do // проходим по всем каталогам
      if not CText(sl[i], path + 'MID_MIF') then // этот каталог не удаляем
        ExShellExecute('cmd', PChar(format('/C rd /S /Q "%s"', [sl[i]])),
          SW_HIDE, true, 0); // удаление каталога
  finally
    sl.Free;
...
type
  ...
  Txls = Record
  // csv : CadastralNumber	ObjectType	DATEFORM(FoundationDate)	NUMBERINPUTINFO	DATEINPUTINFO
    del: Integer;
    ind: Integer;
...
var
  ...
  s, s2, catzem, csvfile, title, footerxml, work, kn, ft: String;
      ...
      begin
        ...
        begin
          ...
          log.Add('Удален дубликат' + xl.CadastralNumber + #9 +
            xl2.FoundationDate + #9 + xl2.NUMBERINPUTINFO + #9 +
            xl2.DATEINPUTINFO);
        end;
        Inc(i);
        xl := list[i];
...
// в качестве параметра дается xls или xlscopy
  ...
  begin
    ...
    i := 0;
    While i < list.Count - 1 do
    // заполняем массив данными из списка уникальнх КН
    begin
      xl := list[i];
      xl2 := list[i + 1];
      ...
      begin
        ...
        While xl.CadastralNumber = Pxls(list[i]).CadastralNumber do
        // двигаемся по дублям
          Inc(i);
    end;
    Clear_xls(list);
...
//      ExcelApp.ActiveWindow.FreezePanes := true;
    ...
    begin // List2Xls
      ...
      try
        ...
        begin
          ...
          sl_.Add(stl[r]);
          sa := SplitStr(stl[r], '~');
          if (c_area > 0) and ((sa[c_area - 1] = '') or (sa[c_area - 1] = '0'))
          then
        ...
        begin
          ...
          ExcelImportText(ExcelApp, TempPath + 'tempsheet.txt', ft,
            '~', '1251');
          SetNumberFormat;
          if part > 1 then
            Workbook.WorkSheets.Item[k].Name :=
              format('Пакет_%dчасть_%d', [k, part]);
        end;
      finally
        sl_.Free;
      end;
    ...
    begin
      ...
      if CText(catzem, 'Buildings') then
        codeobject := '002001002000'
      else if CText(catzem, 'Flats') then
        codeobject := '002001003000'
      else if CText(catzem, 'Constructions') then
        codeobject := '002001004000'
      else if CText(catzem, 'Uncompleteds') then
        codeobject := '002001005000'
      else if CText(catzem, 'CarParkingSpaces') then
        codeobject := '002001009000'
      else if CText(catzem, 'Parcels') then
        codeobject := '002001001000'
      else
      ...
      Result := '<?xml version="1.0" encoding="UTF-8" standalone="yes"?>' +
        ...
        '    <ListType>4</ListType>' + '    <Region>66</Region>' +
        '    <ObjectsType>' + format('      <ObjectType>%s</ObjectType>',
        [codeobject]) + '    </ObjectsType>' + '  </ListInfo>' + '  <Objects>' +
    ...
    var
      ...
      var
        ...
        i, n, m : Integer;
      begin
        if cbCategories.ItemIndex > 0 then // для ОКС не делать
          exit;

        Info(packet + ' Корректировка ЗУ');
        n := 0; m := 0;
        Reader := TStreamReader.Create(work + packet, TEncoding.UTF8);

        packet := ChangeFileExt(packet, 'испр.xml');
        Writer := TStreamWriter.Create(work + packet, false, TEncoding.UTF8);

        try
          while not Reader.EndOfStream do // все в одну строку
          begin
            s := Trim(Reader.ReadLine);

            mc_1 := reg.Match(s, '<SubParcels>.+</SubParcels>', [roMultiLine]);
            if mc_1.Success then
            begin
              s := StringReplace(s, mc_1.Value, '', [rfReplaceAll]);
              Inc(n);
            end;

            mc_1 := reg.Match(s, '<ZonesAndTerritories>.+</ZonesAndTerritories>', [roMultiLine]);
            if mc_1.Success then
            begin
              s := StringReplace(s, mc_1.Value, '', [rfReplaceAll]);
              Inc(m);
...
//        mc := reg.Matches(s, '<SubParcels>.+</SubParcels>',
...
//        mc := reg.Matches(s, '<ZonesAndTerritories>.+</ZonesAndTerritories>',
//          [roIgnoreCase, roMultiLine]);
//        pg.Max := mc.Count;
//        for i := 0 to mc.Count - 1 do
//        begin
...
//        Writer.Free;
      ...
      procedure SplitXML;
      var
        title, catzemplus, dir, partname, s: String;
        i, n, m: Integer;
        mc_1: TMatch;
      begin
        s := '';
        ...
        try
          while not Reader.EndOfStream do
            s := s + Trim(Reader.ReadLine);
        finally
          Reader.Free;
        end;
        s := StringReplace(s, lf, '', [rfReplaceAll]);

        if Length(s) > maxcount then
        begin
          Info('Деление XML...');
          dir := work + 'parts\';
          ForceDirectories(dir);
          mc_1 := reg.Match(s, '<\?xml version=.+?<Objects>\s*<[^>]+>',
            [roSingleLine]);
          if mc_1.Success then
          begin
            title := mc_1.Value;
            ...
            // заголовок удаляем
            n := Pos('</ListForRating>', s);
            mc_1 := TRegEx.Match(s, '<\w+\s+CadastralNumber', [roSingleLine]);
            if mc_1.Success and (n > 0) then
            begin
              catzemplus := mc_1.Value;
              n := PosEx(catzemplus, s, maxcount);
              m := 1;
              while n > 0 do
              begin
                sl.Clear;
                sl.Text := CreateHeaderXML + Copy(s, 1, n - 1) + footerxml;
                partname := format('%s_%d.xml', [ChangeFileExt(packet, ''), m]);
                sl.SaveToFile(dir + partname);

                ExCreateProcess(format('"%s" a -tzip "%s" "%s"',
                  [CatZip.Text, dir + ChangeFileExt(partname, '.zip'),
                  dir + partname]), '', SW_HIDE, false);
          ...
          ExCreateProcess(format('"%s" a -tzip "%s" "%s"',
            [CatZip.Text, work + ChangeFileExt(packet, '.zip'), work + packet]),
            '', SW_HIDE, false);
        Info('');
      end;

      procedure GetNewList(Filename: String);
      var
        j: Integer;
      begin
        ...
        for j := 0 to sl.Count - 1 do
          s := s + Trim(sl[j]);
        sl.Clear;
        s := StringReplace(s, TEncoding.UTF8.GetString(KTPN_UTF8), '',
          [rfReplaceAll]); // удаляем бяки
        s2 := format('</%s>', [catzem]);
        // добавим перевод строк для правильной работы Matches
        s := StringReplace(s, s2, s2 + lf, [rfReplaceAll]);
      end;
    ...
    begin // CreateXML
      ...
      sl_ := TStringList.Create;
      s := ExcelNum2Str(WorkSheet, Col);// row и col  определяется при активации листа
      //обязаиельно надо брать с названием а то при 1 строке data=содержанию ячейки,тоесть строка а не массив
      data := WorkSheet.range[format('%s1:%s%d', [s, s, row])].Value;
      // колонка XML name
      for j := VarArrayLowBound(data, 1)+1 to VarArrayHighBound(data, 1) do
        sl_.Add(String(data[j, 1]));
      VarClear(data);
      data := WorkSheet.range[format('A1:A%d', [row])].Value;
      // колонка кадастровых номеров
      for j := VarArrayLowBound(data, 1)+1 to VarArrayHighBound(data, 1) do
        sl_[j - 2] := sl_[j - 2] + '=' + String(data[j, 1]);
      VarClear(data);
      sl_.Sort;
      pg.Max := sl_.Count;
      size := 0;
      k := 1; n := 0; m := 0;
      try
        GetNewList(sl_.Names[0]);
        ss := sl_.Names[0];
        i := 0;
        while i < sl_.Count do
        begin
          if i mod 10 = 0 then
            pg.Position := i;
          if sl_.Names[i] = ss then
          begin
            kn := sl_.ValueFromIndex[i];
            s2 := format('<%s CadastralNumber="%s".+</%s>', [catzem, kn, catzem]);
            mc_1 := reg.Match(s, s2, [roIgnoreCase, roMultiLine]);
            if mc_1.Success then//выдергиваем из файла блок с КН и нужную категорию земель
            begin
              if size = 0 then
              begin
                part := ChangeFileExt(packet, format('_%d.xml', [k]));
                Writer := TStreamWriter.Create(work + part, false, TEncoding.UTF8);
                Writer.WriteLine(CreateHeaderXML);
              end;

              s2 := mc_1.Value;
              if cbCategories.ItemIndex = 0 then//Parcels
              begin
                mc_1 := reg.Match(s2, '<SubParcels>.+</SubParcels>', [roMultiLine]);
                if mc_1.Success then
                begin
                  s2 := StringReplace(s2, mc_1.Value, '', [rfReplaceAll]);
                  Inc(n);
                end;
                mc_1 := reg.Match(s, '<ZonesAndTerritories>.+</ZonesAndTerritories>', [roMultiLine]);
                if mc_1.Success then
              ...
              begin
                Writer.WriteLine(footerxml);
                Writer.Free;
                ExCreateProcess(format('"%s" a -tzip "%s" "%s"', [CatZip.Text, work + ChangeFileExt(part, '.zip'),
                  work + part]), '', SW_HIDE, false);
                size := 0;
                Inc(k);
              end;
            end;
...
//      CorrectXML; // удаление секций <SubParcels> и <ZonesAndTerritories>
//      SplitXML; // деление на части
      Result := 1;
    end;
    ...
    var
      ...
      r: Integer;
    begin
      Info('Создание файла ' + packet + '.xls');
      r := WorkSheet.UsedRange.Rows.Count;
      range := WorkSheet.range
        [format('A1:%s%d', [ExcelNum2Str(WorkSheet, 5), r])];
      DataParcels := range.Value;
      ...
      try
        ...
        range := WorkSheet_.range
          [format('B3:%s%d', [ExcelNum2Str(WorkSheet, 5 + 1), r + 2])];
        ...
        WorkSheet_.Cells[1, 1].Value := '&T0';
        // заполнение первого столбца (3 строки)
        ...
        Workbook_.SaveAs(Filename := work + packet, FileFormat :=
  ...
  begin // CreateExcel
    ...
    try
      ...
      Repeat
        if not PreparePacket(xlscopy) then
          break;
      ...
      for i := 1 to n do
      begin
        ActivateSheet(i); // активируем лист с нужным пакетом
        CreateTelDaXLS(format('%s_Пакет_%d', [ListObjects[kk], i]));
        // очищаем список (там должен бвть 1 объект)
...
{$IFDEF DEBUG}path := 'D:\Tokyo\SERVICEMAP_VER_14\Win32\Debug\TEST\письма';
  {$ENDIF}// выбрать All
  // {$IFDEF DEBUG}path := 'D:\Tokyo\SERVICEMAP_VER_14\Win32\Debug\TEST\письма\Письма2026_03_12';{$ENDIF}//выбрать All
{$IFDEF DEBUG}path := 'D:\Tokyo\SERVICEMAP_VER_14\Win32\Debug\TEST\большие письма\9999\'; {$ENDIF}// выбрать All
  if not GetFolder(path, Handle) then
    exit; // выбираем каталог All
...
//     for kk := 2 to ListObjects.Count-1 do//tr!!!
    begin
      cn.CommaText := ini.ReadString(ListObjects[kk], 'nodes', '');
      if cn.Count = 0 then
      begin
        ListObjects.Free;
        Exc('Файл ServiceMap.ini не содержит информацию об узлах XML файла для категории '
          + ListObjects[kk]);
      end;
      catzem := path + 'RESULT\' + ListObjects[kk] + '.xlsx';
      if FileExists(catzem) then
        if not DeleteFile(catzem) then
        begin
          ListObjects.Free;
          Exc(format
            ('Файл "%s.xlsx" открыт в Excel.%s Закройте файл и повторите попытку.',
            [catzem, #10]));
        end;
    end;
  ...
  try
    for kk := 0 to ListObjects.Count - 1 do
//     for kk := 1 to ListObjects.Count-1 do//tr!!!
    begin
      footerxml := format('%s%s</%s>%s%s%s</Objects>%s%s</ListForRating>',
      ...
      // формируем список узлов и аттрибутов для колонок EXCEL
      begin
        ...
        begin
          neednode[i].node := Copy(neednode[i].node, 2,
            Length(neednode[i].node));
          Inc(c_add);
          SetLength(neednode[i].check, Length(neednode[i].check) + 1);
          neednode[i].check[Length(neednode[i].check) - 1] :=
            neednode[i].Pos + 1;
      ...
      try
        ...
        begin
          ...
          begin
            ...
            for j := 0 to sl.Count - 1 do
              sl[j] := StringReplace(sl[j], 'CarParkinSpace', 'CarParkingSpace',
                [rfReplaceAll]);
            sl.SaveToFile(xml[i]);
          end;
          sl.Clear;
          ...
          for j := 0 to sl.Count - 1 do
          begin
            mc := reg.Matches(sl[j], '\d\d:\d\d:\d{1,7}:\d{1,}',
              [roIgnoreCase, roMultiLine]);
            if mc.Count = 0 then
            ...
            begin
              ...
              begin // открываем файл CSV и достаем из него данные
                ...
                for n := 1 to csv.Count - 1 do
                // начинаем с 1 записи пропустив заголовок
                  if Pos(mc[0].Value + ';', csv[n]) = 1 then
                  // нашли вхождение КН
                    try
                      xl := AllocMem(SizeOf(xl^));
                      sa := SplitStr(csv[n], ';');
        ...
        begin
          ...
          while PreparePacket(xls) do
          // xls при этом очищается, остается xlscopy
            Inc(k);
          /// ///////////////////создаем копию дублей в StringGrid/////////////////////////
          if xls.Count > 1 then // значит нужен ручной разбор
          begin
            ...
            try // настраиваем форму ручного разбора
              ...
              Manual.Filename := path + 'RESULT\' + ListObjects[kk] +
                '_Ручной разбор.csv';
              ...
              for j := Manual.sg.RowCount - 1 downto 1 do
                ...
                begin
                  xl := xls[j - 1];
                  // номер строки StringGrid соответствует индексу xls-1
                  log.Add('Удален дубликат' + xl.CadastralNumber + #9 +
                    xl2.FoundationDate + #9 + xl2.NUMBERINPUTINFO + #9 +
                    xl2.DATEINPUTINFO); // FoundationDate=DATEFORM
                  k := xl.ind;
    ...
    begin
      ...
      begin
        ...
        sa := SplitStr(s, #9);
        // CadastralNumber,FoundationDate(DATEFORM),NUMBERINPUTINFO,DATEINPUTINFO
    ...
    if log.Count > 0 then
      ExShellExecute('Notepad.exe', path + 'RESULT\' + cbCategories.Text +
        '_log.log', SW_NORMAL, false, 0);
    log.Free;
    ini.Free;
    ListObjects.Free;
    FieldsTypes.Free;
...
const
  XLStitle =
    'registration_type	registration_date	document_date	price	quantity	' +
...
var
  ...
  ExcelApp, Workbook, WorkSheet: OleVariant;
begin
{$IFDEF DEBUG}path := 'D:\Tokyo\SERVICEMAP_VER_14\Win32\Debug\TEST\сделки\';
  {$ENDIF}
  // if path = '' then
  ...
  try
    ...
    begin
      ...
      begin
        ...
        s := StringReplace(s, '</sale_contract>', '</sale_contract>' + lf,
          [rfReplaceAll]);
        sa := SplitStr(XLStitle, #9);
        mc := reg.Matches(s, '<sale_contract>.+</sale_contract>',
          [roIgnoreCase, roMultiLine]); // блоки
        ...
        begin
          ...
          for n := Low(sa) to High(sa) do
          begin
            mc2 := reg.Matches(mc.Item[j].Value, '<' + sa[n] + '>.+?</' + sa[n]
              + '>', []); // секции (? ленивый поиск)
            if mc2.Count > 0 then
            ...
            begin
              ...
              if k > 0 then
              begin
                m := PosEx('"', mc.Item[j].Value, k + Length(ss));
                // закрывающая кавычка
                if m > 0 then
                  s2 := s2 + Copy(mc.Item[j].Value, k + Length(ss),
                    m - k - Length(ss)) + #9;

                if sa[n] = 'quantity' then
                  quantity := StrToInt(Copy(mc.Item[j].Value, k + Length(ss),
                    m - k - Length(ss)));
                if sa[n] = 'cadastral_number' then
                // кадастровый номер для замены
                  cn := Copy(mc.Item[j].Value, k + Length(ss),
                    m - k - Length(ss));
          ...
          if quantity > 1 then
          begin
            mc2 := reg.Matches(mc.Item[j].Value, '\d\d:\d\d:\d{1,7}:\d{1,}', []
              ); // кадастровые номера
        ...
        begin
          ...
          try
            ...
            ExcelApp.Workbooks.OpenText(Filename := '"' + xmsname + '"',
              Origin := 1251, StartRow := 1, DataType := xlDelimited,
              TextQualifier := xlDoubleQuote, ConsecutiveDelimiter := false,
              tab := true, Other := false, TrailingMinusNumbers := true);
            WorkSheet := ExcelApp.ActiveSheet;
...
{$IFDEF DEBUG}PrepareForm.PacketPath := 'D:\Tokyo\SERVICEMAP_VER_14\Win32\Debug\TEST\тест_правка_FD\2\SOKS_Zu_a_2026-02-19_1'; {$ENDIF}
  ...
  begin
    ...
    begin
      s := format('/c del /F /Q  "%s"', [TempPath + '*.*']);
      // удалим и пересоздадим
      ...
      begin
        ...
        // список коротких имен FD со ссылкой на полные
        ss := '';
        for j := 0 to cn.Count - 1 do
        begin
        ...
        Names.SaveToFile(sub + 'НСПД\' + ExtractFileName(DellSlesh(sub))
          + '.txt');
        Names.Free;
        ...
        for j := 0 to cn.Count - 1 do
        // раскладываем по каталогам для проверки уникальности
        begin
          s := TempPath + AddSlesh(IntToStr(j));
          ForceDirectories(s);
          ExCreateProcess(format('"%s" x "%s" -y -o"%s"', [CatZip.Text, cn[j],
            s]), '', SW_HIDE, true);
        end;
        ...
        try
          ...
          for j := 0 to sl.Count - 1 do
          // создаем список коротких имен для сортировки
          ...
          begin
            ...
            begin
              s := Copy(Names[j], 1, Length(Names[j]) - 4) +
                format('(%d).xml', [k]);
        ...
        sl := TStringList.Create;
        Names := TStringList.Create;
        // список коротких имен COST со сылкой на полные
        try
...
// рабочий каталог MapInfo  C:\Users\TroitskikhVV\AppData\Local\Temp\MIPRO
  ...
  begin
    ...
    begin
      ...
      fd.AddObject(sl[i], xml);
      // название xml и ссылка на его содержимое в объекте
      mc := reg.Matches(xml.Text,
        '<CadastralNumber>\d\d:\d\d:\d{1,7}:\d{1,}</CadastralNumber>',
        [roIgnoreCase, roMultiLine]);
      kn := TStringList.Create; // список кадастровых номеров
      for j := 0 to mc.Count - 1 do
      begin
        k := Pos('</CadastralNumber>', mc.Item[j].Value);
        // конец кадастрового номера
...
begin // CorrectFDproc
  ...
  sl := TStringList.Create;
  fd := TStringList.Create;
  // список списков название xml и ссылка на его содержимое в объект
  cn := TStringList.Create;
  // список списков кадастровых номеров в xml (симметричен fd)
  packet := TStringList.Create; // список каталогов в каталоге "ПАКЕТЫ"
  cat := TStringList.Create;
  // список каталогов содержащих Приложение к Акту АОКС
  log := TStringList.Create;
  try
    ...
    for i := 0 to sl.Count - 1 do // FD.zip формируется а модуле PrepareFD_Unit
      if CText(ExtractFileName(sl[i]), 'FD.zip') then // разархивация файлов XML
        ExCreateProcess(format('"%s" x "%s" -y -o"%s"', [CatZip.Text, sl[i],
          TempPath]), '', SW_HIDE, true);
    ...
    ExShellExecute('cmd', PChar(format('/C rd /S /Q "%s"',
      [path + 'SOKC_Result'])), SW_HIDE, true, 0);
    // удаление каталога со всем содержимым и подкаталогами
    ForceDirectories(path + 'SOKC_Result');
    // создаем каталог для сохранения расчетов
    ExShellExecute('cmd', format('/C xCopy /y /s /e "%s" "%s"',
      // копируем исходные данные из пакетов
    ...
    for k := sl.Count - 1 downto 0 do
      if Pos('Проверочный', ExtractFileName(sl[k])) = 1 then
        DeleteFile(sl[k])
      else // удалим проверочные файлы, они не нужны
    ...
    begin
      ...
      begin
        Info(format('пакет %s каталог %s', [ExtractFileName(packet[i]),
          ExtractFileName(cat[j])]));
        sl.Clear;
        FindRecursive(AddSlesh(cat[j]), '*.xlsx', false, sl);
        if sl.Count = 0 then
          Continue;
        xlsname := sl[0];
        // должен быть единственный файл - это наше приложение в акту
        sheetname := GetSheetNameFirst; // берем название первого листа в книге

        tab := CreateTableFromExcel(sl[0], sheetname, '');
        // создаем таблицу Mapinfo из Приложение к Акту АОКС
        miDo('Select * from %s where Кадастровая_стоимость__руб<>"" group by N_PROCEDURE into Proc',
          [tab]);
        n_proc := miEvalInt('TableInfo(Proc, %d)', [TAB_INFO_NROWS]);
        if n_proc > 0 then // число уникальных номеров процедур
        begin
          for n_proc := 1 to miEvalInt('TableInfo(Proc, %d)', [TAB_INFO_NROWS])
            do // идем по группам процедур
          begin
            ...
            if sl.Count > 0 then
            begin
              for n := 1 to miEvalInt('TableInfo(%s, %d)',
                [tab, TAB_INFO_NROWS]) do
              begin
              ...
              begin
                ...
                begin
                  log.Add(kn[k] + ' удален');
                  ss := format('<CadastralNumber>%s</CadastralNumber>',
                    [kn[k]]);
              ...
              log.Add(format
                ('Для процедуры "%s" файлы "FD_???.XML" не найдены!!!',
                [proc]));
          end;
          DeleteFile(xlsname);
          ...
          if sl.Count > 0 then
          begin
            nom := ExtractFileName(sl[0]);
            // АОКС-66-2024-002005_от_23.09.2024_на_2780.ods
          ...
          begin
            ...
            begin
              if (Length(nom) = 10) and (Pos('FD', ExtractFileName(sl[k])) = 1)
              then
              begin
                xml := TStringList.Create;
                try
                  ...
                  begin
                    mc := reg.Matches(xml[l],
                      '<Date_Doc>\d\d\d\d-\d\d-\d\d</Date_Doc>',
                      [roIgnoreCase, roMultiLine]);
                    if mc.Count > 0 then
                    ...
                    mc := reg.Matches(xml[l],
                      '<Report_Details Date="\d\d\d\d-\d\d-\d\d"',
                      [roIgnoreCase, roMultiLine]);
                    if mc.Count > 0 then
                      xml[l] := StringReplace(xml[l], mc.Item[0].Value,
                    ...
                    mc := reg.Matches(xml[l],
                      '<N_Doc>АОКС-\d\d/\d\d\d\d/\d\d\d\d\d\d</N_Doc>',
                      [roIgnoreCase, roMultiLine]);
                    if mc.Count > 0 then
                      xml[l] := StringReplace(xml[l], mc.Item[0].Value,
                    ...
                    mc := reg.Matches(xml[l],
                      'Number="АОКС-\d\d/\d\d\d\d/\d\d\d\d\d\d">',
                      [roIgnoreCase, roMultiLine]);
                    if mc.Count > 0 then
                      xml[l] := StringReplace(xml[l], mc.Item[0].Value,
          ...
          log.Add(format
            ('пакет %s каталог %s: НЕТ ДАННЫХ "Кадастровая_стоимость__руб"',
            [ExtractFileName(packet[i]), ExtractFileName(cat[j])]));
        miDo('Drop table %s', [tab]);
      end;
...
begin
  ...
  Data := VarArrayCreate([1, VarArrayHighBound(DataParcels, 1), 1, n - id],
    varVariant);
  ...
  begin
    ...
    miDo('Fetch Rec %d From %s', [i, tab]);
    miDo('ob = %s.obj', [tab]);
    miDo('Select * from %s Where %s.Obj Contains ob Into TempTab NoSelect',
      [t, t]);
    ...
    begin // по названию поля выбираем значения
      ...
      begin
        ...
        begin
          s := Copy(fields.ValueFromIndex[id + j - 1], 10, 100);
          // выбираем таблицу и поле после Contains
          sa := SplitStr(s, ';'); // =fname
        ...
        if CText(fname, 'НаименованиеНП_МО') then
          Data[i, 1] := StringReplace(String(Data[i, 1]), 'ё', 'е',
...
begin
  ...
  begin
    ...
    miDo('Select * from %s Where %s.Obj Intersects ob Into TempTab NoSelect',
      [t, t]);
    ...
    begin
      ...
      begin
        ...
        s := Copy(fields.ValueFromIndex[id + j - 1], 12, 100);
        // выбираем таблицу и поле после Intersects
        sa := SplitStr(s, ';'); // =fname
...
var
  ...
  colarea, colcena, colcomment, colprocent, collat, collon, colpdf, coladr,
    coladr2, colpred: Integer; // нужны для работы
...
// поиск КН найденных в Дежурной карте
  ...
  begin
        ...
        begin
          ...
          for k := 0 to ds.FieldDefs.Count - 1 do
            if Pos('DATECREATED', ds.FieldDefs[k].Name) = 1 then
              s := s + Copy(ds.FieldByName(ds.FieldDefs[k].Name).AsString,
                1, 10) + '~'
            else
    ...
    Data := VarArrayCreate([1, VarArrayHighBound(DataParcels, 1), 1, n],
      varVariant);
    for i := 0 to sl.Count - 1 do
    begin
      sa := SplitStr(sl[i], '~');
  ...
  begin
    ...
    try
      ...
      begin
        ...
        for i := VarArrayLowBound(semantik, 2) to VarArrayHighBound
          (semantik, 2) do
        ...
        for i := 0 to ds.FieldDefs.Count - 1 do
        begin
          WorkSheet.Cells[1, col2 + i] :=
            format('%s_%d', [ds.FieldDefs[i].Name, k]);
          if CText('SEGMENTOFUSAGE', ds.FieldDefs[i].Name) then
            WorkSheet.Columns[col2 + i].NumberFormat := '@';
        end;
        ...
        begin
          ...
          begin
            for j := 0 to ds.FieldDefs.Count - 1 do
              if Pos('DATECREATED', ds.FieldDefs[k].Name) = 1 then
                Data[1, j + 1] :=
                  Copy(ds.FieldByName(ds.FieldDefs[j].Name).AsString, 1, 10)
              else
...
begin // TServiceMapForm.CalcOne
  ...
  try
    ...
    for i := 0 to fields.Count - 1 do
      ...
      else if CText(fields.Names[i], 'Distance_Analogs') then
  ...
  try
    ...
    if tabdbf = '' then
    // нет готового файла от Штаня делаем все вычисления сами
    begin
      Info('Подготовка данных...');
      ...
      // берем размеры на созданном листе
      row := WorkSheet.UsedRange.Rows.Count;
      k := 0;
      for i := 0 to fields.Count - 1 do
      ...
      begin
        ...
        range := WorkSheet.range
          [format('A2:%s%d', [ExcelNum2Str(WorkSheet, n), row])];
      end;
      // -------------отдельный запуск программы CompareStrings.exe--------------------
      if FindColValue(n, '07') and FindColValue(m, '007') and
        FindColValue(k, '0007') then
      begin
        colprocent := n; // AQ
        ExShellExecute('cmd', format('/C xCopy /y /s /e "%s" "%s"',
          [source, TempPath]), SW_HIDE, true, 0);
        source := TempPath + ExtractFileName(source);
      ...
      for j := VarArrayLowBound(DataParcels, 1) to VarArrayHighBound
        (DataParcels, 1) do
      ...
      ExCreateProcess(format('"%sCopyFiles.exe" "%s" "%s" "%s" "%s"',
        [GetAppFolder, TempPath + 'files.txt', xml, AddSlesh(path), respathPDF,
        Handle]), '', SW_MINIMIZE, false);
      // ------------------------------------------------------------------------------
      Info('Проверка данных');
      for i := VarArrayLowBound(DataParcels, 1) to VarArrayHighBound
        (DataParcels, 1) do
        for j := VarArrayLowBound(DataParcels, 2) to VarArrayHighBound
          (DataParcels, 2) do
        begin
          pg.Position := i;
          ...
          begin
            s := DataParcels[i, j];
            s := StringReplace(StringReplace(s, #10, ' ', [rfReplaceAll]), #13,
              ' ', [rfReplaceAll]);
            s := StringReplace(StringReplace(s, 'ё', 'е', [rfReplaceAll]), 'Ё',
              'Е', [rfReplaceAll]);
      ...
      begin
        ...
        begin
          pg.Position := i;
          Data[i, 1] := String(DataParcels[i, collat]) + ' ' +
            String(DataParcels[i, collon]);
          DataParcels[i, Col] := Data[i, 1];
          // для Заполнение группировки площадей
        end;
        range.Value := Data;
        Workbook.Save;
        VarClear(Data);
      ...
      Workbook.SaveAs(Filename := txt, FileFormat := xlText,
        CreateBackup := false);
      ExcelApp.Workbooks.Close;
      ...
      miDo('Register Table "%s" TYPE ASCII Delimiter 9 Titles Charset "WindowsCyrillic" Into "%s"',
        [txt, s]);
      ...
      begin
        ...
        pg.Max := VarArrayHighBound(DataParcels, 1);
        Data := VarArrayCreate([1, VarArrayHighBound(DataParcels, 1), 1, 2],
          varVariant);
        ...
        begin
          ...
          Val(s, xy.y, m);
          Wgs2Msk_(xy.y, xy.x, 2);
          miDo('Create Point Into Variable ob	(%.9f, %.9f) Symbol MakeSymbol(34,%d,%d)',
            [xy.x, xy.y, RGB(0, 0, 255), 10]);
      ...
      begin
        if Pos('Contains', fields.ValueFromIndex[i]) = 1 then
          try
            ...
            sa := SplitStr(s, ';');
            if FileExists(analogs + sa[0] + '.tab') then
            // полное название таблицы
              Contains(i, row, kodnp, idkp, fields, WorkSheet, DataParcels, tab,
                analogs + sa[0] + '.tab', sa[1])
            else
      ...
      begin
        if Pos('Intersects', fields.ValueFromIndex[i]) = 1 then
          try
            ...
            sa := SplitStr(s, ';');
            if FileExists(analogs + 'Дороги_WGS84\' + sa[0] + '.tab') then
            // полное название таблицы
              Intersects(i, row, radius, fields, WorkSheet, DataParcels, tab,
                analogs + 'Дороги_WGS84\' + sa[0] + '.tab', sa[1])
            else
              log.Add(format('Файл %s не найден',
                [analogs + 'Дороги_WGS84\' + sa[0] + '.tab']));
          finally
            SetLength(sa, 0);
          end;
        Inc(i);
      ...
      // ------------------------------------------------------------------------------
      if FindColValue(n, '01') and FindColValue(k, '001') and
        FindColValue(m, '4') then
        if k = n + 1 then
        begin
          Info('Группировка по координатам');
          s := TempPath + 'group.txt';
          miDo('Select COL%d,Count(*) from %s group by COL%d into TempTab NoSelect',
            [m, tab, m]); // объединенные координаты
          miDo('Select * From TempTab where Count > 1 Into group  NoSelect');
          miDo('Export group Into "%s" Type "ASCII" Overwrite Delimiter "=" CharSet "WindowsCyrillic"',
            [s]);
          miDo('Close table TempTab');
      ...
      if FindColValue(n, '05') and FindColValue(k, '005') then
        ...
        begin
          ...
          miDo('Create Table TempBuffer (ID Integer, Area_Dist Float) File "%sTempBuffer.tab"',
            [TempPath]);
          ...
          xy.z := miEvalDbl('CartesianArea(ob, "sq m")') + 0.1;
          // площадь для сравнения буферов
          ...
          miDo('Select * from TempBuffer into selBuffer Where CartesianArea(obj, "sq m")>%.f',
            [xy.z]);
          ...
          begin
            ...
            miDo('Fetch rec %d From %s', [i, tab]);
            miDo('ob = %s.obj', [tab]);
            // выбираем буфер в котором находится объект
      ...
      if FindColValue(n, '02') and FindColValue(k, '002') and
        FindColValue(m, '0002') then
        ...
        begin
          ...
          miDo('Select COL%d,Count(*) from %s group by COL%d into TempTab NoSelect',
            [colarea, tab, colarea]);
          miDo('Select * From TempTab where Count > 1 Into group  NoSelect');
          miDo('Export group Into "%s" Type "ASCII" Overwrite CharSet "WindowsCyrillic"',
            [s]);
          miDo('Close table TempTab');
          ...
          Data := VarArrayCreate([1, VarArrayHighBound(DataParcels, 1), 1, 3],
            varVariant);
          for i := VarArrayLowBound(DataParcels, 1) to VarArrayHighBound
            (DataParcels, 1) do
          begin
      ...
      // -----------------------------УПКС=--------------------------------------------
      if FindColValue(n, '06') and FindColValue(k, '006') and
        FindColValue(m, '0006') then
        if (k = n + 1) and (k = m - 1) then
        begin
          ...
          miDo('Update TempTab Set COL1 = Round(COL1/Col2, 1)');
          // округляем до целого
          ...
          miDo('Export TempTab Into "%s" Type "ASCII" Overwrite CharSet "WindowsCyrillic"',
            [s]);
          sl.LoadFromFile(s);
      ...
      // ------------------------------------------------------------------------------
      if FindColValue(n, '03') and FindColValue(k, '003') and
        FindColValue(i, '0003') then
        if (k = n + 1) and (k = i - 1) and (idkp <> 0) and (kodnp <> 0) then
        begin
          Info('Создание буферных зон...');
          SafeCloseTable('TempBuffer');
          miDo('Create Table TempBuffer (ID Integer, Count Integer, Area Float) File "%sTempBuffer.tab"',
            [TempPath]);
          miDo('Create Map For TempBuffer CoordSys Earth Projection 1, 104');
          for i := VarArrayLowBound(DataParcels, 1) to VarArrayHighBound
            (DataParcels, 1) do
            if (Pos(':000:0:0:0', String(DataParcels[i, kodnp])) = 3) and
              (String(DataParcels[i, idkp]) = '') then
            begin
              miDo('Fetch Rec %d From %s', [i, tab]);
              miDo('ob = %s.obj', [tab]);
          ...
          begin
            ...
            begin
              ...
              miDo('select * from %s where ob Contains %s.obj into temptab noSelect',
                [tab, tab]);
              s := miEval('TableInfo(temptab, %d)', [TAB_INFO_NROWS]);
              // число объектов попавших в буфер
      ...
      // ------------------------------------------------------------------------------
      if FindColValue(n, '04') and FindColValue(k, '004') and
        FindColValue(m, '0004') then
        if (k = n + 1) and (k = m - 1) then
        begin
          Info('Группировка по комментариям');
          s := TempPath + 'group.txt';
          miDo('Select Комментарий,count(*) from %s group by Комментарий into TempTab NoSelect',
            [tab]);
          miDo('Select * From TempTab where Count > 1 Into group  NoSelect');
          miDo('Export group Into "%s" Type "ASCII" Overwrite CharSet "WindowsCyrillic"',
            [s]);
      ...
      begin
        for j := 0 to fields.Count - 1 do
          if AnsiContainsText(fields.ValueFromIndex[j], 'НаименованиеНП_МО')
          then
          begin
            col2 := j + 1; // колонка naim_np
            Info('Проверяем Соответствие НП');
            pg.Max := VarArrayHighBound(DataParcels, 1);
            Data := VarArrayCreate([1, VarArrayHighBound(DataParcels, 1), 1, 1],
              varOleStr);
            for i := VarArrayLowBound(DataParcels, 1)
              to VarArrayHighBound(DataParcels, 1) do
            begin
              pg.Position := i;
              s := WorkSheet.Cells[i + 1, col2];
              // для WorkSheet заголовок пропускаем
              if ContainsText(DataParcels[i, coladr], s) or
                ContainsText(DataParcels[i, coladr2], s) or
                ContainsText(DataParcels[i, colpred], s) or
                ContainsText(DataParcels[i, colcomment], s) then
                Data[i, 1] := 'Да'
              else
                Data[i, 1] := 'Нет';
            end;
      ...
      if FileExists(ExtractFilePath(source) + 'text.txt') and (colprocent <> 0)
      then
      begin
      ...
      s := ExtractFilePath(xls) + 'Z' + ExtractFileName(DellSlesh(respathPDF)) +
        'graf.dbf';
    ...
    else // есть готовый файл от Штаня
    begin
      Info('Копирование исключений...');
      Col := WorkSheet.UsedRange.Columns.Count; // размеры на листе
      row := WorkSheet.UsedRange.Rows.Count;
      miDo(fieldsdbf);
      // выбор из исправленной таблицы (от Штаня) сформированный в Path_Unit
      col2 := 1;
      for i := 1 to Length(fieldsdbf) do
      // подсчитываем количество колонок через разделитель(запятую)
        if fieldsdbf[i] = ',' then
          Inc(col2);
      miDo('Export tempTab Into "%stempTab.txt" Type "ASCII" Overwrite CharSet "WindowsCyrillic" Titles',
        [TempPath]);
      sl.LoadFromFile(TempPath + 'tempTab.txt');
      for i := 0 to sl.Count - 1 do
        sl[i] := StringReplace(sl[i], '"', '', [rfReplaceAll]);

      Data := VarArrayCreate([1, row, 1, col2], varVariant);
      // массив для заполнеия данными с заголовком
...
begin
  ...
  try
    ...
    // D:\Tokyo\SERVICEMAP_VER_12\Win32\Debug\Аналоги_test4\Аналоги_группа.txt
    if not OpenDialog1.Execute then
      exit;
    OpenDialog1.InitialDir := ExtractFilePath(OpenDialog1.Filename);
    ...
    xls := GetAppFolder + 'Аналоги\' + AddSlesh(CurrentData) +
      'Общий_Результат.xlsx';
    ...
    if FileExists(s) then
      if not DeleteFile(s) then
        Exc(format('Файл "%s" открыт.%s Закройте файл и повторите попытку.',
          [s, #10]));
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          ExShellExecute('cmd', PChar(format('/C Copy /Y "%s" "%s"', [xls, s])),
            SW_HIDE, true, 0);
          xls := s;
          ExcelApp.Workbooks.Open(xls);
          WorkSheet := ExcelApp.Workbooks.Item[1].WorkSheets.Item[1];
          Col := WorkSheet.UsedRange.Columns.Count;
          // берем размеры на новом листе
          ...
          try
            ...
            // активируем первую книгу (первый рабочий лист становится активным)
...
begin
  ...
  try
    ...
    for i := fields.Count - 1 downto 0 do
    begin
      if (Pos('=Contains', fields[i]) = 0) and
        (Pos('=Intersects', fields[i]) = 0) then
        fields.Delete((i));;
    end;
  finally
    ini.Free;
...
var
  ...
  begin
    s := Data;
    Result := StringReplace(StringReplace(s, #10, ' ', [rfReplaceAll]), #13,
      ' ', [rfReplaceAll]);
...
{$IFDEF DEBUG} OpenDialog1.InitialDir := 'D:\Tokyo\SERVICEMAP_VER_12\Win32\Debug\Аналоги_test\DK_SEMANTIC'; {$ENDIF}
  ...
  try
    ...
    ExCreateProcess(format('"%s" x "%s" -y -o"%s"', [ss, OpenDialog1.Filename,
      TempPath]), '', SW_HIDE, true);
    ...
    begin
      ...
      for i := 6 to row do
        try
          ...
          ds.Close;
          ds.CommandText :=
            format('Select %s from geo.dk_semantic where CADASTRALNUMBER=''%s''',
            [F, String(Data[1, index[0]])]);
          ds.Open;
          if ds.IsEmpty then // новая запись
          begin
            cmd := format('Insert into geo.DK_SEMANTIC (%s) values(''%s'')',
              [F, String(Data[1, index[0]])]);
            for j := 1 to fields.Count - 1 do
              System.Insert(format(',''%s''', [str_(Data[1, index[j]])]), cmd,
                Length(cmd));
            RunCommand(cmd);
            Inc(newrow);
          end
          ...
          begin
            for j := 1 to fields.Count - 1 do
            begin
              s := ds.FieldByName(fields.Names[j]).AsString;
              // строка из базы, DATECREATED: yyyy-mm-dd
              ss := Data[1, index[j]];
              // строка из Excel, DATECREATED: dd.mm.yyyy
              if CText(fields.Names[j], 'DATECREATED') then
              begin
                if s <> '' then
                begin
                  sa := SplitStr(Copy(s, 1, 10), '-');
                  // строка всегда не пустая
                  s := sa[2] + '.' + sa[1] + '.' + sa[0];
                  SetLength(sa, 0);
                end;
                if ss = '' then
                begin
                  ...
                  log.Add(String(Data[1, index[0]]) +
                    ' DATECREATED нет данных');
                end;
              end;
              ...
              begin
                ...
                for n := 1 to fields.Count - 1 do
                  cmd := cmd + format('%s = ''%s'',',
                    [fields.Names[n], str_(Data[1, index[n]])]);
                cmd := Copy(cmd, 1, Length(cmd) - 1) +
                  format(' Where CADASTRALNUMBER = ''%s''',
    ...
    log.Add(format('Добавлено записей %d%sОбновлено записей %d',
      [newrow, #10, upprow]));
...
{$IFDEF DEBUG} OpenDialog1.InitialDir := 'D:\Tokyo\SERVICEMAP_VER_12\Win32\Debug\Аналоги_test\XML'; {$ENDIF}
  ...
  try
    ...
    n := Pos('<Objects>', s);
    n := PosEx('<', s, n + 12);
    // добавляем длину больше длины <Objects> и сразу находим начало catzem
    m := PosEx(' ', s, n + 1);
    ...
    sl.SaveToFile(s + s2);
    sl.Free;
    Note(format('Удалено:%s SubParcels %d%s ZonesAndTerritories %d',
      [#10, n, #10, m]))
  end;
end;
...
begin
  ...
  // D:\Tokyo\SERVICEMAP_VER_12\Win32\Debug\TEST\от_Шмелева_Сводный_Результат\Сводный_Результат_2.xlsx
  if not OpenDialog1.Execute then
    exit;
  OpenDialog1.InitialDir := ExtractFilePath(OpenDialog1.Filename);
  ExcelApp := CreateOleObject('Excel.Application');
  ...
  try
    ...
    alert := '';
    Workbook.WorkSheets.Add(Before := ExcelApp.ActiveSheet, Count := 1);
    // вставляем новый лист перед активным
    n := 1;
    for i := 0 to sl.Count - 1 do
    begin
    ...
    ExcelApp.Visible := true;
    if alert <> '' then
      Note('Не найдены колонки:' + #10 + alert +
        'Проверьте секцию [FIELDS_USER]');
  finally
    ExcelApp := Unassigned;
    Workbook := Unassigned;
    WorkSheet := Unassigned;
